...
from reid_strong_baseline.layers import CrossEntropyLabelSmooth, TripletLoss
...
parser.add_argument(
    "--dataset", default="coco", type=str, metavar="DATASET", help='Name of model to train (default: "coco"'
)
parser.add_argument(
    "--model",
...
parser.add_argument("--val-skip", type=int, default=0, metavar="N", help="Skip every N validation samples.")
parser.add_argument(
    "--num-classes",
    type=int,
    default=None,
...
parser.add_argument("--clip-grad", type=float, default=10.0, metavar="NORM", help="Clip gradient norm (default: 10.0)")

# Optimizer parameters
parser.add_argument("--opt", default="momentum", type=str, metavar="OPTIMIZER", help='Optimizer (default: "momentum"')
parser.add_argument("--opt-eps", default=1e-3, type=float, metavar="EPSILON", help="Optimizer Epsilon (default: 1e-3)")
parser.add_argument("--momentum", type=float, default=0.9, metavar="M", help="SGD momentum (default: 0.9)")
parser.add_argument("--weight-decay", type=float, default=4e-5, help="weight decay (default: 0.00004)")
...
parser.add_argument("--eval-metric", default="map", type=str, metavar="EVAL_METRIC", help='Best metric (default: "map"')
parser.add_argument(
    "--tta",
    type=int,
    default=0,
...
parser.add_argument(
    "--reid_config",
    default="data/processed/Peta/configs/softmax_triplet_with_center.yml",
    type=str,
    metavar="PATH",
    help="path to reid config",
)
parser.add_argument("--xbm_epoch", type=int, default=0, help=" epoch when cross batch memory start")

parser.add_argument("--reid_loss_weight", type=float, default=1, help="weight for reid loss on batch")
parser.add_argument("--reid_xbm_loss_weight", type=float, default=1, help="weight for reid loss on cross batch memory")
parser.add_argument("--class_loss_weight", type=float, default=0.1, help="weight for cross entropy loss")
parser.add_argument("--xbm_size", type=int, default=200, help="size of cross batch memory buffer")
parser.add_argument("--reid_feature_dim", type=int, default=256, help="dimension of reid features ")
...
def main():
    ...
    output_uri = "s3://astralai-trains/videoanal/efficientdet"  # path for saving models (torch.save) with clearml hooks
    ...
    (
        ...
        num_classes_reid,
        num_query,
    ) = create_datasets_and_loaders(args, model_config, reid_cfg=cfg)
    ...
    reid_evaluator = create_supervised_evaluator(
        ReidEvalBench(model),
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if model_ema is not None:
                ...
                eval_metrics = validate(
                    writer, epoch, model_ema.module, loader_eval, args, evaluator, log_suffix=" (EMA)"
                )
            else:
...
def create_datasets_and_loaders(
    args, model_config, transform_train_fn=None, transform_eval_fn=None, collate_fn=None, reid_cfg=None
):
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                ...
                if loss_xbm is not None:
                    writer.add_scalar(
                        "Loss_step/reid_xbm", losses_reid_xbm_m.val, global_step=(epoch * last_idx + batch_idx)
                    )
    ...
    return OrderedDict(
        [
            ("loss", losses_m.avg),
            ("loss-reid", losses_reid_m.avg),
            ("loss-class", losses_class_m.avg),
            ("loss-det", losses_det_m.avg),
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if epoch % 5 == 0 and batch_idx == 0:
                ...
                for i in range(len(images)):
                    ...
                    images[i] = torchvision.utils.draw_bounding_boxes(
                        images[i], boxes, colors=[(255, 0, 0)] * len(boxes)
                    )
