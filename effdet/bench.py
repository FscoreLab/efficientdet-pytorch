...
class ReidBench(nn.Module):
    def __init__(self, model, num_classes, neck, neck_feat):
        ...
        self.fuse_1 = nn.Sequential(nn.Conv2d(self.base.config.fpn_channels,
        ...
        self.fuse_3 = nn.Sequential(nn.ConvTranspose2d(self.base.config.fpn_channels, 256, 2, 2, 0, 0), nn.Conv2d(256, 256, 3, padding=1),
                                    nn.BatchNorm2d(256))
    ...
    def reid_forward(self, x, target):
        ...
        idx = torch.arange(0, global_feat.shape[0]).long().expand((4, global_feat.shape[0])).T.flatten()
