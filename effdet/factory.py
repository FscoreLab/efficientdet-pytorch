...
def create_model(
        ...
        checkpoint_path='', checkpoint_ema=False, predict_uncertainties=False, confluence=False, confidence_thresh=.5, use_mask_branch=False,
        use_custom_seresnet=False,
        **kwargs):
    ...
    return create_model_from_config(
        ...
        confluence=confluence, confidence_thresh=confidence_thresh, use_mask_branch=use_mask_branch, use_custom_seresnet=use_custom_seresnet,
        **kwargs)
...
def create_model_from_config(
    ...
    model = EfficientDet(config, pretrained_backbone=True, use_segm_branch=use_mask_branch, use_custom_seresnet=use_custom_seresnet)  # pretrained_backbone
    ...
    create_labeler = True
    if bench_task == 'train':
        model = DetBenchTrain(model, create_labeler=create_labeler, predict_uncertainties=predict_uncertainties)
    elif bench_task == 'predict':
        model = DetBenchPredict(model, predict_uncertainties=predict_uncertainties, confluence=confluence, confidence_thresh=confidence_thresh,
         **kwargs)
    return model
